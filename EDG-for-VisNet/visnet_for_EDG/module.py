...
from visnet_for_EDG.distillation_utils import *
from visnet_for_EDG.models.model import create_model, load_model
...
class LNNP(LightningModule):
    def __init__(self, hparams, prior_model=None, mean=None, std=None):
        ...
        if self.hparams.load_model:
            self.model, state_dict = load_model(self.hparams.load_model, args=self.hparams)
        else:
        ...
        self.mean_all, self.std_all, self.threshold_all = None, None, None

    def configure_optimizers(self):
        model_param_group = [{"params": self.model.parameters(), "lr": self.hparams.lr, "weight_decay": self.hparams.weight_decay},
                             {"params": self.alignMapper.parameters(), "lr": self.hparams.lr, "weight_decay": self.hparams.weight_decay}]
        optimizer = AdamW(model_param_group)
    ...
    def forward(self, data, ret_mol_repr=False):
        return self.model(data, ret_mol_repr)

    def distillation(self, molecule_3D_repr, data, hparams, mean_all=None, std_all=None, threshold_all=None):
        self.alignMapper.train()
        self.EDPredictor.eval()

        device = data.y.device
        img_feat = torch.from_numpy(np.stack(data['img_feat'])).to(device)

        ED_loss = torch.zeros(1).to(device)
        if hparams.use_ED:
            molecule_3D_repr = self.alignMapper(molecule_3D_repr)  # 映射到和 image 相同的维度

        if hparams.use_ED:
            ED_graph = self.EDPredictor(molecule_3D_repr)
            ED_image = self.EDPredictor(img_feat)
    ...
    def step(self, batch, loss_fn, stage):
        # print(f"[{stage}] batch...", len(batch))
        with torch.set_grad_enabled(stage == "train" or self.hparams.derivative):
            if stage == "train" and self.hparams.weight_ED:
                ret_mol_repr = True
            else:
                ret_mol_repr = False
            ...
            if stage == "train" and ret_mol_repr:
                ED_loss = self.distillation(mol_repr, batch, self.hparams)
                self.losses[stage + "_ED"].append(ED_loss.detach())
        ...
        loss = (loss_y * self.hparams.energy_weight + loss_dy * self.hparams.force_weight + self.hparams.weight_ED * ED_loss)
        
        self.losses[stage].append(loss.detach())

        if stage == "train":
            self.log("loss_y", loss_y * self.hparams.energy_weight, prog_bar=True)
            self.log("loss_dy", loss_dy * self.hparams.force_weight, prog_bar=True)
            self.log("ED_loss", ED_loss * self.hparams.weight_ED, prog_bar=True)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if not self.trainer.sanity_checking:
            ...
            if len(self.losses["train_y"]) > 0 and len(self.losses["train_dy"]) > 0:
                ...
                result_dict["train_loss_ED"] = torch.stack(self.losses["train_ED"]).mean()
